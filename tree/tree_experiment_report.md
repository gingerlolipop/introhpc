...
We simulate how trees colonize a two-dimensional landscape over discrete time steps. The landscape is a rectangular grid: each cell is either empty or occupied by one tree. Starting from **two clusters** of 100 founder trees each, separated along the horizontal axis, trees spread outward cycle by cycle according to a simple dispersal rule. Over 12 cycles on a 1200 × 1200 grid, the population grows from 200 founders to 35 526 trees.
...
Before the first cycle, `numClusters` founder clusters are placed on the landscape. Each cluster receives `treesPerCluster` trees drawn by rejection sampling within a small box around its centre. Cluster centres are spaced evenly along the column axis, symmetric about the grid midpoint, with `clusterSeparation` cells between adjacent centres. In the benchmark run, two clusters of 100 trees each are separated by 200 cells on the 1200 × 1200 grid.
...
| Cycles | 12 |
| Clusters | 2 |
| Trees per cluster | 100 |
| Cluster separation | 200 cells |
| Dispersal radius | 15 |
| Random seed | 12345 |
...
**Serial baseline** (`tree2.chpl`): **0.345 s** mean, final tree count **35 526**.
...
| 4 | 0.168 | 2.05 | 35 526 |
| 8 | 0.143 | 2.41 | 35 526 |
| 16 | 0.129 | 2.68 | 35 526 |
| 32 | 0.096 | 3.58 | 35 526 |
...
With twice as many founders spread across two separated clusters, the population reaches 35 526 trees by cycle 12 — nearly double the 17 779 trees produced by a single central cluster of 100 under the same seed and radius. The two fronts expand independently at first and merge as their dispersal disks overlap.

Parallelism becomes beneficial beyond 1 thread: speedup reaches **3.6×** at 32 threads, reducing runtime from 0.345 s (serial) to 0.096 s.

At 1 thread, the parallel executable is **20% slower** than the serial code (0.430 s vs 0.345 s). This overhead reflects the cost of `forall` scheduling, atomic operations, and parallel reduction infrastructure that are unnecessary at single-thread concurrency.

Scaling is near-linear from 1 to 8 threads (0.80× → 2.41×) but sub-linear beyond that (2.68× at 16 threads, 3.58× at 32). The per-cell neighbourhood scan is memory-bound and repeats work across overlapping disks, so adding threads yields diminishing returns once the grid update is sufficiently saturated.
...
**Visualization parameters.** Frames use a **360 × 360** grid (reduced from 600 × 600) with the same seed, radius, and cluster layout as the benchmark (two clusters of 100 trees, 60-cell separation). The smaller landscape raises the occupied fraction to about **21%** by cycle 12 (26 848 trees), making the spread easier to see. Benchmark timing still uses 1200 × 1200.
...
Snapshots at cycles 0, 3, 6, 9, and 12 show two founder clusters (dark green) and successive dispersal waves in distinct colours. The legend on the right identifies each birth cycle.
...
<p align="center">
  <img src="viz/figures/cycle_000.png" width="220" alt="Cycle 0 — two clusters of 100 founder trees"/>
  <img src="viz/figures/cycle_003.png" width="220" alt="Cycle 3 — 1 431 trees"/>
  <img src="viz/figures/cycle_006.png" width="220" alt="Cycle 6 — 6 765 trees"/>
  <img src="viz/figures/cycle_009.png" width="220" alt="Cycle 9 — 15 472 trees"/>
  <img src="viz/figures/cycle_012.png" width="220" alt="Cycle 12 — 26 848 trees"/>
</p>
...
| `tree_scaling_45699789.csv` | Scaling summary (CSV) |
